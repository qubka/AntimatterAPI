...
public class WorldGenStone extends WorldGenBase<WorldGenStone> {
    ...
    public WorldGenStone(String id, StoneType type, int amount, int size, int probability, int minY, int maxY, RegistryKey<World>... dims) {
...
//                long hash = (world.getDimensionKey() & 0xffL) << 56 | ((long) chunkX & 0x000000000fffffffL) << 28 | (long) chunkZ & 0x000000000fffffffL;
...
//                    int x = chunkX * 16;
//                    int z = chunkZ * 16;
//                    rand.setSeed(world.getSeed() ^ ((world.getDimensionKey() & 0xffL) << 56 | ((long) x & 0x000000000fffffffL) << 28 | (long) z & 0x000000000fffffffL) + Math.abs(0/*mBlockMeta*/) + Math.abs(size) + (block.getType().getId().equals("granite_red") || block.getType().getId().equals("granite_black") ? 32768 : 0));  //Don't judge me
